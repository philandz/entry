...
impl EntryRepository {
    ...
    pub async fn create_entry(
        ...
        self.create_entry_full(budget_id, category_id, kind, amount, description, entry_date, tags, notes, created_by, None, None, None).await
    }
    ...
    pub async fn create_entry_full(
        ...
        let next_occ = if is_recurring { Some(entry_date.to_string()) } else { None };
        sqlx::query(
    ...
    pub async fn list_entries(&self, budget_id: Option<&str>, budget_ids: &[String], user_id: Option<&str>, params: &ListParams) -> Result<ListResult> {
        ...
        if let Some(bid) = budget_id {
            ...
            for id in budget_ids { binds.push(id.clone()); }
        ...
        if let Some(ref kind) = params.kind { if !kind.is_empty() { conditions.push("e.kind = ?".to_string()); binds.push(kind.clone()); } }
        if let Some(ref cat) = params.category_id { if !cat.is_empty() { conditions.push("e.category_id = ?".to_string()); binds.push(cat.clone()); } }
        if let Some(ref df) = params.date_from { if !df.is_empty() { conditions.push("e.entry_date >= ?".to_string()); binds.push(df.clone()); } }
        if let Some(ref dt) = params.date_to { if !dt.is_empty() { conditions.push("e.entry_date <= ?".to_string()); binds.push(dt.clone()); } }
        if let Some(min) = params.amount_min { conditions.push("e.amount >= ?".to_string()); binds.push(min.to_string()); }
        if let Some(max) = params.amount_max { conditions.push("e.amount <= ?".to_string()); binds.push(max.to_string()); }
        ...
        for b in &binds { data_q = data_q.bind(b); }
        ...
        Ok(ListResult {
            entries,
            meta: PageMeta { page, page_size, total_pages: total_pages.max(1), total_rows: total },
        })
    }
    ...
    pub async fn update_entry(
        ...
        if category_id.is_some() { parts.push("category_id = ?".to_string()); }
        if kind.is_some()        { parts.push("kind = ?".to_string()); }
        if amount.is_some()      { parts.push("amount = ?".to_string()); }
        if description.is_some() { parts.push("description = ?".to_string()); }
        if entry_date.is_some()  { parts.push("entry_date = ?".to_string()); }
        if tags.is_some()        { parts.push("tags = ?".to_string()); }
        if notes.is_some()       { parts.push("notes = ?".to_string()); }
        let sql = format!("UPDATE budget_entries SET {} WHERE id = ? AND deleted_at IS NULL", parts.join(", "));
        let mut q = sqlx::query(&sql).bind(now);
        if let Some(v) = category_id { q = q.bind(v); }
        if let Some(v) = kind        { q = q.bind(kind_to_db(v)); }
        if let Some(v) = amount      { q = q.bind(v); }
        if let Some(v) = description { q = q.bind(v); }
        if let Some(v) = entry_date  { q = q.bind(v); }
        if let Some(v) = tags        { q = q.bind(v.join(",")); }
    ...
    pub async fn delete_entry(&self, entry_id: &str) -> Result<()> {
        ...
        sqlx::query("UPDATE budget_entries SET deleted_at = ?, updated_at = ? WHERE id = ?")
            .bind(now).bind(now).bind(entry_id)
            .execute(&self.pool).await?;
        Ok(())
    }
    ...
    pub async fn get_entry_budget_id(&self, entry_id: &str) -> Result<Option<String>> {
        ...
        ).bind(entry_id).fetch_optional(&self.pool).await?;
        Ok(row.map(|(v,)| v))
    }
    ...
    pub async fn add_comment(&self, entry_id: &str, body: &str, created_by: &str) -> Result<DbComment> {
        let id = new_id();
        let now = now_unix();
    ...
    pub async fn delete_comment(&self, comment_id: &str) -> Result<()> {
        ...
        sqlx::query("UPDATE entry_comments SET deleted_at = ?, updated_at = ? WHERE id = ?")
            .bind(now).bind(now).bind(comment_id)
            .execute(&self.pool).await?;
        Ok(())
    }
    ...
    pub async fn get_comment_entry_id(&self, comment_id: &str) -> Result<Option<String>> {
        ...
        ).bind(comment_id).fetch_optional(&self.pool).await?;
        Ok(row.map(|(v,)| v))
    }
    ...
    pub async fn attach_file(&self, entry_id: &str, file_id: &str, file_name: &str, created_by: &str) -> Result<DbAttachment> {
        let id = new_id();
        let now = now_unix();
    ...
    pub async fn remove_attachment(&self, attachment_id: &str) -> Result<()> {
        sqlx::query("DELETE FROM entry_attachments WHERE id = ?")
            .bind(attachment_id).execute(&self.pool).await?;
        Ok(())
    }
    ...
    pub async fn get_attachment_entry_id(&self, attachment_id: &str) -> Result<Option<String>> {
        let row: Option<(String,)> = sqlx::query_as(
            "SELECT entry_id FROM entry_attachments WHERE id = ?"
        ).bind(attachment_id).fetch_optional(&self.pool).await?;
        Ok(row.map(|(v,)| v))
    }
    ...
    pub async fn update_recurrence_rule(&self, entry_id: &str, rule: Option<&str>) -> Result<DbEntry> {
        let now = now_unix();
    ...
    pub async fn advance_next_occurrence(&self, entry_id: &str, next_date: &str) -> Result<()> {
        let now = now_unix();
        sqlx::query(
            "UPDATE budget_entries SET next_occurrence = ?, updated_at = ? WHERE id = ?"
        )
        .bind(next_date).bind(now).bind(entry_id)
        .execute(&self.pool).await?;
        Ok(())
    }
