...
pub fn map_entry(db: DbEntry) -> Entry {
    Entry {
        ...
        tags:            db.tags.unwrap_or_default()
                            .split(',')
                            .filter(|s| !s.is_empty())
...
pub fn map_comment(db: DbComment) -> Comment {
    Comment {
        id: db.id, entry_id: db.entry_id, body: db.body,
        created_by: db.created_by, created_at: db.created_at, updated_at: db.updated_at,
    }
}

pub fn map_attachment(db: DbAttachment) -> Attachment {
    Attachment {
        id: db.id, entry_id: db.entry_id, file_id: db.file_id,
        file_name: db.file_name, created_by: db.created_by, created_at: db.created_at,
    }
}
